...
class Decoder(srd.Decoder):
    ...
    id = 'adxl345_spi'
    name = 'ADXL345 SPI'
    longname = 'Analog Devices ADXL345 SPI'
    desc = 'ADXL345 3-axis accelerometer'
    license = 'gplv2+'
    inputs = ['spi']
    outputs = []
    tags = ['IC', 'Sensor']
    ...
    def get_axis_value(self, data, axis):
        if self.data != - 1:
            data <<= 8
            self.data |= data
            self.put(self.start_index, self.es, self.out_ann, [4, ['%s: 0x%04X' % (axis, self.data), str(data)]])
            self.data = -1
        else:
            self.putx([4, [str(data)]])

    def handle_reg_0x1D(self, data):
        self.handle_reg_with_scaling_factor(data, 62.5, 'Threshold', 'g', error_messages['undesirable'])
    ...
    def handle_reg_0x21(self, data):
        self.handle_reg_with_scaling_factor(data, 0.625, 'Time', 's', error_messages['dis_single_double'])

    def handle_reg_0x22(self, data):
        self.handle_reg_with_scaling_factor(data, 62.5, 'Latent', 's', error_messages['dis_double'])

    def handle_reg_0x23(self, data):
        self.handle_reg_with_scaling_factor(data, 1.25, 'Latent', 's', error_messages['dis_double'])

    def handle_reg_0x24(self, data):
        self.handle_reg_with_scaling_factor(data, 62.5, 'Latent', 's', error_messages['undesirable'])

    def handle_reg_0x25(self, data):
        self.handle_reg_0x1D(data)

    def handle_reg_0x26(self, data):
        self.handle_reg_with_scaling_factor(data, 1000, 'Time', 's', error_messages['interrupt'])
    ...
    def handle_reg_0x29(self, data):
        self.handle_reg_with_scaling_factor(data, 5, 'Time', 's', error_messages['undesirable'])
    ...
    def handle_reg_0x2A(self, data):
        bits = [Bit('', BitType.UNUSED),
                ...
                Bit('', BitType.UNUSED),
                Bit('', BitType.UNUSED),
                Bit('', BitType.OTHER, {1: ['Suppressed', 'Suppr', 'S'], 0: ['Unsuppressed', 'Unsuppr', 'Uns'],}),
    ...
    def handle_reg_0x2B(self, data):
        bits = [Bit('', BitType.UNUSED),
                ...
                Bit('', BitType.OTHER, {1: ['Asleep', 'Asl'], 0: ['Not asleep', 'Not asl', '!Asl'],}),
    ...
    def handle_reg_0x2C(self, data):
        ...
        self.putbs([4, ['%f' % rate_code[rate]]], self.reverse_bit_index(stop_index, WORD_SIZE),
    ...
    def handle_reg_0x2D(self, data):
        ...
        self.putbs([4, ['%d Hz' % frequency]], self.reverse_bit_index(stop_index, WORD_SIZE),
                   self.reverse_bit_index(start_index, WORD_SIZE))
    ...
    def handle_reg_0x31(self, data):
        ...
        result = 2 ** (range_g + 1)
        self.putbs([4, ['+/-%d g' % result]], self.reverse_bit_index(stop_index, WORD_SIZE),
                   self.reverse_bit_index(start_index, WORD_SIZE))
    ...
    def handle_reg_0x38(self, data):
        ...
        self.putbs([4, [fifo_modes[fifo]]], self.reverse_bit_index(stop_index, WORD_SIZE),
                   self.reverse_bit_index(start_index, WORD_SIZE))

        start_index = 0
        stop_index = 4
        samples = self.get_decimal_number(bits_values, start_index, stop_index)
        self.putbs([4, ['Samples: %d' % samples, '%d' % samples]], self.reverse_bit_index(stop_index, WORD_SIZE),
                   self.reverse_bit_index(start_index, WORD_SIZE))
    ...
    def handle_reg_0x39(self, data):
        ...
        self.putbs([4, ['Entries: %d' % entries, '%d' % entries]], self.reverse_bit_index(stop_index, WORD_SIZE),
                   self.reverse_bit_index(start_index, WORD_SIZE))

    def get_bit(self, channel):
        if (channel == Channel.MOSI and self.mosi is None) or (channel == Channel.MISO and self.miso is None):
        ...
        if ptype == 'CS-CHANGE':
            ...
            if self.state == 'ADDRESS-BYTE':
                ...
                for i in range(6):
                    ...
                    self.address <<= 1
                self.address >>= 1
                self.put(start_sample, addr_bit[2], self.out_ann, [3, ['ADDRESS: 0x%02X' % self.address, 'ADDR: 0x%02X'
